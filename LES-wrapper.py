...
directory, computes ROC-AUC, the optimal-F1 threshold, and Best-F1 at each
checkpoint, then integrates these into a single Learning Efficiency Score (LES)
per metric - the area under the metric-vs-epoch curve.

This is the ppiBTEP port of the ppiGPLM / ppiDCE LES-wrapper. The model-specific
glue that differs here:

  1. Checkpoints. ppiBTEP saves one checkpoint per epoch as
     `ppiBTPE_epoch_{N}.pth` (note the underscore before the number) plus
     `ppiBTPE_final.pth`. The x-axis of every LES curve is the training *epoch*.

  2. Inference engine. ppiBTEP uses `inference_ppiBTPE_2GPU.py`, which
     **requires** a `--num_layers` argument (the SiameseBTPE config is rebuilt
     with that many transformer layers) and supports multi-GPU via
     `--device cuda:0,1`.

  3. Inference output format. ppiBTEP writes columns
     ...
     column named `Probability_Friends`, which is the **second-to-last** column
     (Probability_Enemies is last). This wrapper reads `Probability_Friends` by
     name (with a positional second-to-last fallback). Contrast ppiDCE, whose
     positive-class `prob_1` was the *last* column.
