#!/usr/bin/env python
"""
LES-wrapper.py — Learning Efficiency Score evaluation across ppiBTEP checkpoints.

Runs ppiBTEP (SiameseBTPE) inference on PRS (Positive Reference Set) and RRS
(Random Reference Set) sequence-pair files at every saved checkpoint in a
directory, computes ROC-AUC and Best-F1 at each checkpoint, then integrates these
into a single Learning Efficiency Score (LES) per metric — the area under the
metric-vs-epoch curve.

This is the ppiBTEP port of the LES-wrapper family. Two things about it:

  1. Model-specific glue. ppiBTEP saves one checkpoint per epoch as
     `ppiBTPE_epoch_{N}.pth` (note the underscore before the number) plus
     `ppiBTPE_final.pth`, so the x-axis of every LES curve is the training
     *epoch*. Inference uses `inference_ppiBTPE_2GPU.py`, which **requires** a
     `--num_layers` argument (the SiameseBTPE config is rebuilt with that many
     transformer layers) and supports multi-GPU via `--device cuda:0,1`. Its
     output columns are `seq1, seq2, [label,] Prediction, Probability_Friends,
     Probability_Enemies`; the positive-class ("friends" = interacting = label 1)
     probability is `Probability_Friends`, the **second-to-last** column.

  2. Output shape follows ppiGPLM's LES-wrapper_v2.py (the deliberate port
     target). Relative to the earlier ppiBTEP/ppiDCE wrapper, the outputs here
     therefore:
       ...
       - emit a vector **PDF** alongside every PNG;
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='LES-wrapper: Learning Efficiency Score evaluation across ppiBTEP checkpoints',
...
Examples:
  python LES-wrapper.py --checkpoint_dir ROC_Checkpoints \\
      ...
      --output_dir LES_results --num_layers 6 --include_final
        """
    )

    # Required arguments
    parser.add_argument('--checkpoint_dir', type=str, required=True,
                        help='Directory containing ppiBTEP checkpoints (ppiBTPE_epoch_*.pth)')
    ...
    parser.add_argument('--checkpoint_pattern', type=str, default='ppiBTPE_epoch_*.pth',
                        help='Glob to match per-epoch checkpoints (default: ppiBTPE_epoch_*.pth)')
    parser.add_argument('--include_final', action='store_true',
                        help='Also evaluate ppiBTPE_final.pth (plotted after the last epoch; '
                             'excluded from the LES integral)')

    # ppiBTEP inference settings (forwarded to inference_ppiBTPE_2GPU.py)
    parser.add_argument('--inference_script', type=str, default=None,
                        help='Path to inference_ppiBTPE_2GPU.py (default: alongside this wrapper)')
    parser.add_argument('--model_config', type=str,
                        default='facebook/esm1b_t33_650M_UR50S',
                        help='ESM model name or local path for tokenizer/config')
    parser.add_argument('--num_layers', type=int, default=6,
                        help='Transformer layers used at training time (required by the '
                             'inference script for config override). The ppiBTEP V3 '
                             'checkpoints are 6-layer, so default is 6; pass the value '
...
def extract_epoch_from_checkpoint(ckpt_name):
    """Extract the epoch number from a ppiBTEP checkpoint filename."""
    # Match ppiBTPE_epoch_1.pth, ppiBTPE_epoch_12.pth, etc. (underscore optional).
    match = re.search(r'epoch[_-]?(\d+)\.pth$', ckpt_name)
    if match:
        return int(match.group(1))
    # Final checkpoint sorts after every numbered epoch.
    if ckpt_name == 'ppiBTPE_final.pth':
        return float('inf')
    return None
...
def get_checkpoints(checkpoint_dir, pattern, include_final=False):
    ...
    if include_final:
        final_ckpt = os.path.join(checkpoint_dir, 'ppiBTPE_final.pth')
...
def run_inference(inference_script, ckpt_path, input_file, output_csv,
                  model_config, num_layers, batch_size, max_length, device):
    """Run ppiBTEP inference for one checkpoint, writing directly to output_csv."""
    cmd = [
    ...
    print(f"  Running: inference_ppiBTPE_2GPU.py --model_path {os.path.basename(ckpt_path)} "
          f"--num_layers {num_layers} --input_file {os.path.basename(input_file)} ...")
...
def extract_probabilities_from_csv(csv_path):
    """Extract the positive-class probability from a ppiBTEP inference CSV.

    ppiBTEP writes columns:
        seq1, seq2, [label,] Prediction, Probability_Friends, Probability_Enemies
    The positive class ("friends" = interacting = label 1) probability is the
    column named `Probability_Friends`, i.e. the SECOND-to-last column
    (Probability_Enemies is last). Sequences contain no commas, so a positional
    fallback to the second-to-last column is safe if the header is ever missing.
    """
    probabilities = []
    ...
    with open(csv_path, 'r', newline='') as f:
        ...
        for row in reader:
            if not row:
                continue
            try:
                probabilities.append(float(row[pos_idx]))
...
def write_analysis_readme(output_dir):
    ...
    text = """# LES analysis — plot legend (ppiBTEP)

This folder is the Learning Efficiency Score (LES) analysis for one ppiBTEP
(SiameseBTPE) model evaluated on one PRS (Positive Reference Set) / RRS (Random
Reference Set) pair, across all saved training checkpoints (one per epoch).
**PRS = blue (positives); RRS = red (negatives).** The interaction score for each
pair is `Probability_Friends` in [0, 1] — see the next section for exactly what
that is.
...
the ppiBTEP classifier's **softmax probability for the "friends" (interacting)
class**.

ppiBTEP encodes each protein with its Siamese ESM tower and the classifier head
emits 2 logits over the classes `{enemies, friends}`. A softmax turns those into a
probability for each class and we read off the mass on `friends`:

```
P(interaction) = softmax(classifier logits)[ "friends" ] = Probability_Friends
```

- It is a genuine 2-class softmax, so **`Probability_Friends + Probability_Enemies = 1`**
  exactly. Both are saved in each per-checkpoint `*_probabilities.csv`.
- **It is `Probability_Friends` for *both* PRS and RRS** — a single, shared score
  function, *not* "friends-prob for PRS and enemies-prob for RRS". That shared score is
  what makes the ROC/AUC valid: every pair gets the same score `P(friends)`, PRS pairs
  are labelled 1 and RRS pairs 0, and AUC measures how well `P(friends)` ranks true
  interactors above random ones. A good model pushes `P(friends)` high for PRS (blue,
  near 1) and low for RRS (red, near 0). `Probability_Enemies` is recorded but is
  **not** used for AUC, F1, LES, or any plot.
...
def main():
    ...
    # Locate the ppiBTEP inference script (defaults to alongside this wrapper).
    script_dir = os.path.dirname(os.path.abspath(__file__))
    inference_script = args.inference_script or os.path.join(script_dir, 'inference_ppiBTPE_2GPU.py')
    ...
    print(f"\n{'='*60}")
    print("LES-wrapper: Learning Efficiency Score Evaluation (ppiBTEP)")
    print(f"{'='*60}")
    ...
    for idx, (ckpt_name, epoch, ckpt_path) in enumerate(checkpoints):
        ...
        if not args.skip_inference:
            ...
            if not run_inference(inference_script, ckpt_path, args.prs_file, prs_csv,
                                 args.model_config, args.num_layers, args.batch_size,
        ...
        if args.no_metrics:
            # Random controls: no true positives, so AUC/F1/ROC are meaningless.
    ...
    if have_metrics:
        ...
        if args.no_metrics:
            ...
            print(f"\nOnly {len(epochs)} checkpoint(s) analyzed — skipping LES / trajectories "
                  f"(need >= 2 checkpoints).")
    ...
    print(f"  Saved summary table to {summary_csv}"
          + ("  (AUC/Best_F1 columns blank: random control, metrics excluded)"
             if args.no_metrics else ""))
