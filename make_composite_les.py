#!/usr/bin/env python3
"""
make_composite_les.py — Composite (N-model ensemble) LES figures for ppiYYD.

Aggregates the per-run LES-wrapper.py outputs of the 10 V3 replicate models into
three publication-quality composite views, mirroring the ppiGPLM 10-model
ensemble (Enselmble_10_V3_models), but on ppiYYD's *epoch* axis and its
`Probability_Friends` score:

  1. composite_trajectory_AUC(.png/.pdf)   — across-model MEAN ROC-AUC per epoch,
     with a ±1 SD band; title reports ensemble LES-AUC = mean ± SD.
  ...
  3. composite_PRS-RRS_violins(.png/.pdf)  — the PRS vs RRS P(interaction)
     distributions POOLED across all N models at each epoch (each model
     contributes its own 100 reference pairs -> N*100 pooled values / epoch).

Plus composite_trajectory_data.csv (per-epoch mean/SD + LES mean/SD) and two
legend .md files.

Inputs are the per-run LES-wrapper output directories (one per model), each of
which must contain `summary_table.csv` and `epoch_<N>/{PRS,RRS}_epoch<N>_probabilities.csv`.
By default it discovers `LES_V3-*` subdirectories under --parent.

Example
-------
    python make_composite_les.py \\
        --parent results/ensemble_10models \\
        --run_glob 'LES_V3-*' \\
        --out results/ensemble_10models/composite
"""
import argparse
import csv
import glob
...
PUB_DPI = 600
AUC_C = "#1f4e9c"   # blue  (matches the per-model LES-wrapper AUC colour)
...
# -----------------------------------------------------------------------------
# Discover the per-run LES directories
# -----------------------------------------------------------------------------
def _trailing_int(s):
    m = re.search(r'(\d+)\s*$', s)
...
def find_run_dirs(parent, run_glob):
    dirs = [d for d in glob.glob(os.path.join(parent, run_glob))
            if os.path.isdir(d) and os.path.exists(os.path.join(d, 'summary_table.csv'))]
    if not dirs:
        raise SystemExit(f"No run dirs with summary_table.csv match "
                         f"'{run_glob}' under {parent}")
    dirs.sort(key=lambda d: _trailing_int(os.path.basename(d)))
    return dirs


# -----------------------------------------------------------------------------
# Trajectories (AUC / Best-F1): across-model mean ± SD, ensemble LES
# -----------------------------------------------------------------------------
def read_summary(run_dir):
    ...
    with open(os.path.join(run_dir, 'summary_table.csv')) as fh:
        for row in csv.DictReader(fh):
            ...
            if not ep.isdigit():   # per-epoch rows only (any checkpoint prefix)
                continue
...
def make_trajectory(epochs, mat, color, metric_name, les_vals, base, floor=False):
    """mat: (n_models, n_epochs). floor=False -> ±1 SD band; floor=True -> area
    shading to the floor with ±1 SD error bars on each node."""
    mean = mat.mean(axis=0)
    sd = mat.std(axis=0, ddof=0)              # population SD across the models
    n = mat.shape[0]
    ...
    ax.set_title(
        f"{metric_name} trajectory — {n}-model V3 ensemble\n"
...
def trajectories(run_dirs, out):
    ...
    # common epoch grid across all models (intersection), sorted
    common = sorted(set.intersection(*[set(s) for s in summaries]))
    if len(common) < 2:
        raise SystemExit(f"Need >= 2 shared epochs across models; got {common}")
    dropped = [sorted(set(s) - set(common)) for s in summaries]
    if any(dropped):
        print(f"  epoch grid intersected to {common}; per-model dropped: {dropped}")
...
# -----------------------------------------------------------------------------
# Violins: PRS vs RRS P(interaction) pooled across models per epoch
# -----------------------------------------------------------------------------
def read_pos_probs(path, pos_col):
    """Positive-class probability (Probability_Friends) per pair from a
    ppiYYD LES-wrapper per-checkpoint probabilities CSV. Reads the named column;
    falls back to the second-to-last column if the header lacks it."""
    vals = []
...
def violins(run_dirs, epochs, out, pos_col):
    ...
    # PRS and RRS counts differ when homodimers are depleted from PRS only; that
    # is expected. Warn only if a group's own count varies across epochs.
    if not (all(len(v) == n_prs for v in prs) and all(len(v) == n_rrs for v in rrs)):
    ...
    ax.set_title(f"{len(run_dirs)}-model composite — PRS vs RRS\n"
                 f"pooled across V3-1…V3-{len(run_dirs)}; "
                 f"PRS n = {n_prs}, RRS n = {n_rrs} per epoch",
                 fontsize=14, pad=30)
    fig.tight_layout()
    ...
    plt.close(fig)
    print(f"  wrote composite_PRS-RRS_violins.png/.pdf  (PRS n={n_prs}, RRS n={n_rrs}/epoch)")


# -----------------------------------------------------------------------------
def write_legends(out, n_models):
    traj = f"""# Composite LES trajectories — legend

`composite_trajectory_AUC.*` and `composite_trajectory_F1.*` show the
**across-model mean** of ROC-AUC / Best-F1 at each training epoch for the
{n_models}-model V3 ensemble, with a shaded **±1 SD** band (population SD, ddof=0).
The `_area` variants shade the area under the mean curve and draw the SD as error
bars on each node. Each per-model value is read from that model's
`summary_table.csv` (produced by `LES-wrapper.py`); the per-model Learning
Efficiency Score (LES) is the area under its own metric-vs-epoch curve (epochs
min-max normalized to [0,1], trapezoidal rule). The title reports the ensemble
LES as **mean ± SD** across the {n_models} models. `composite_trajectory_data.csv`
holds the per-epoch mean/SD and the LES mean/SD.
"""
    viol = f"""# Composite PRS/RRS violins — legend

`composite_PRS-RRS_violins.*` pools the per-pair interaction score
`P(interaction) = Probability_Friends` across all {n_models} models at each
training epoch — each model contributes its own 100 matched reference pairs, so
each violin summarises {n_models}*100 pooled values. Left section: PRS
(positives, blue) — should sit high. Right section: RRS (negatives, red) —
should sit low. Black line = median. y-axis is the ppiYYD decoder's 2-class
softmax mass on the "friends" (interacting) class, in [0, 1].
"""
...
def main():
    ap = argparse.ArgumentParser(
        description="Composite (N-model ensemble) LES figures for ppiYYD.")
    ap.add_argument("--parent", required=True,
                    help="Directory containing the per-run LES-wrapper output dirs.")
    ap.add_argument("--run_glob", default="LES_V3-*",
                    help="Glob for the per-run LES dirs under --parent (default: LES_V3-*).")
    ap.add_argument("--out", default=None,
                    help="Output directory (default: <parent>/composite).")
    ...
    out = args.out or os.path.join(args.parent, "composite")
    os.makedirs(out, exist_ok=True)

    run_dirs = find_run_dirs(args.parent, args.run_glob)
    print(f"Composite over {len(run_dirs)} models:")
    for d in run_dirs:
    ...
    write_legends(out, len(run_dirs))
    print(f"DONE -> {out}")
